...
const getFrameworkDescription = (framework: FrameworkType) => {
  switch (framework) {
    ...
    case 'mediation': return 'Conflict resolution methodology';
    case 'decision-matrix': return 'Systematic decision evaluation';
    case 'interest-based-negotiation': return 'Focus on underlying interests';
    case 'feedforward-coaching': return 'Future-focused feedback approach';
    case 'responsibility-mapping': return 'RACI role clarification';
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">Step {path.length + 1} of 2</h3>
          ...
          {currentNode.options.map((option, index) => (
            <div key={index}>
              <button
                ...
                className="w-full p-6 text-left border border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all duration-200 group"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    {(option.category || option.next) && (
                      <div className={`p-3 bg-gradient-to-r ${getCategoryColor(option.category || option.next || '')} rounded-lg text-white shadow-sm`}>
                        {getCategoryIcon(option.category || option.next || '')}
                      </div>
                    )}
                    <div className="text-left">
                      <span className="font-semibold text-gray-900 group-hover:text-blue-900 block text-lg">
                        {option.text}
                      </span>
                      {option.framework && (
                        <span className="text-sm text-gray-600 group-hover:text-blue-700 mt-1 block">
      ...
      {isFirstLevel && (
        <div className="bg-amber-50 rounded-2xl p-6 border border-amber-200">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h4 className="font-semibold text-amber-900 mb-3">Need help choosing?</h4>
