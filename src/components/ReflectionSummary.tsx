...
export function ReflectionSummary({ reflection, onEdit, onDelete, onExport, onViewCompletion }: ReflectionSummaryProps) {
  ...
  const generatePDF = async () => {
    ...
    // Enhanced function to add wrapped text with automatic page breaks
    const addWrappedTextWithPageBreaks = (text: string, x: number, y: number, maxWidth: number, fontSize: number = 10, isBold: boolean = false) => {
      pdf.setFontSize(fontSize);
      pdf.setFont('helvetica', isBold ? 'bold' : 'normal');
      const lines = pdf.splitTextToSize(text, maxWidth);
...
Please provide specific, actionable advice based on my reflection responses. Focus on practical next steps I can take as a leader.`;
    ...
    pdf.setFontSize(8);
    pdf.setFont('helvetica', 'normal');
    yPosition = addWrappedTextWithPageBreaks(aiContent, margin, yPosition, contentWidth, 8, false);
    
    // Add some spacing after the AI content
    yPosition += 10;
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.text(`www.reflacto.com | Professional Leadership Development`, margin, pageHeight - 10);
...
I'm sharing a leadership reflection from my private development tool "Reflacto" and would like your insights and analysis.
